...
module.exports = (email, password, branch, ptr, logAmount) => {
  return gulpCollect.list((fileVinyls, cb) => {
    let cbd = Q.defer();
    _gulpUploadVinylsAsModules(fileVinyls, cbd.makeNodeResolver(), email, password, branch, ptr, logAmount);
    return cbd.promise;
  });
}
...
function _gulpUploadVinylsAsModules(fileVinyls, cb, email, password, branch, ptr, logAmount) {
  let modules = {}
  ...
  let req = https.request({
    ...
    port: 443,
    path: ptr ? '/ptr/api/user/code' : '/api/user/code',
    method: 'POST',
    auth: `${email}:${password}`,
    headers: {
