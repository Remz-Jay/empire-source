...
StructureTower.prototype.run = function(): boolean {
	const damagedCreeps = this.room.myCreeps.filter((c: Creep) => !_.includes(this.room.towerTargets, c) && c.hits < c.hitsMax);
	if (damagedCreeps.length > 0) {
		const target = _.sortBy(damagedCreeps, "hits").shift();
		this.room.towerTargets.push(target);
		this.heal(target);
		return true;
	}
	if (this.room.hostileCreeps.length > 0) {
		const hostiles: Creep[] = this.pos.findInRange(this.room.hostileCreeps, global.getTowerRange(this.room.name));
		const filteredHostiles: Creep[] = hostiles.filter((c: Creep) =>
	...
	if (this.energy > (this.energyCapacity / 2)) {
		const minHits: number = this.room.controller.level * 11000;
		...
		if (damagedStructures.length > 0) {
			const target = _.sortBy(damagedStructures, "hits").shift();
			this.room.towerTargets.push(target);
			this.repair(target);
			return true;
		}
